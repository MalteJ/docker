...
// Return the IPv4 (and IPv6) address of a network interface
func GetIfaceAddr(name string, enableIPv6 bool) (net.Addr, net.Addr, error) {
	...
	if enableIPv6 {
		switch {
		case len(addrs6) == 0:
			return nil, nil, fmt.Errorf("Interface %v has no IPv6 addresses", name)
		case len(addrs6) > 1:
			fmt.Printf("Interface %v has more than 1 IPv6 address. Defaulting to using %v\n",
				name, (addrs6[0].(*net.IPNet)).IP)
		}
		return addrs4[0], addrs6[0], nil
	}
	return addrs4[0], nil, nil
}
